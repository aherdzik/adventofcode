...
function LightCommand(type, point1, point2) {
    this.inputType=type;
...
function parsePoint(input){
	...
	return new Point(splitString[0],splitString[1]);
}
...
var splitString= toInput.split("|");
for(var i=0; i<splitString.length; i++){
	var curString= splitString[i].split(" ");
	var curCommand= new LightCommand(0,0,0);
	if(curString[0] == "turn"){
		if(curString[1] == "on"){
			curCommand.inputType=1;
		}
		else if(curString[1] == "off"){
			curCommand.inputType=2;
		}
		else{
			console.log("ERROR");
		}
		curCommand.upperLeft = parsePoint(curString[2]);
		curCommand.lowerRight = parsePoint(curString[4]);
	}
	else{
		curCommand.upperLeft = parsePoint(curString[1]);
		curCommand.lowerRight = parsePoint(curString[3]);
	}
	commands.push(curCommand);
	console.log(curCommand.inputType + " " + curCommand.upperLeft.x + " " + curCommand.upperLeft.y + " " + curCommand.lowerRight.x + " " + curCommand.lowerRight.y);
}
runProgram();*/
...
function runProgram(){
	for(var i= 0; i<commands.length; i++){
		...
		if(curCommand.inputType==0){
			toggle(curCommand.upperLeft,curCommand.lowerRight);
		}
		else if(curCommand.inputType==1){
			turn(curCommand.upperLeft,curCommand.lowerRight,true);
		}
		else if(curCommand.inputType==2){
			turn(curCommand.upperLeft,curCommand.lowerRight,false);
		}
		else{
			console.log("ERROR");
		}
...
function toggle(upperLeft,lowerRight){
	
	for(var i=upperLeft.y; i<=lowerRight.y; i++){
		
		for(var j=upperLeft.x; j<=lowerRight.x; j++){
			lightArray[j][i]=!lightArray[j][i];
		}
	}
}

function turn(upperLeft,lowerRight,isOn){
